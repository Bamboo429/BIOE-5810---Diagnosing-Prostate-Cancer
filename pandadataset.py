...
class PANDADataset(Dataset):
    ...
    def get_tile_from_paths(self, paths):
        ...
        #    break
            ...
            #tiles = np.array([cv2.cvtColor(cv2.imread(p), cv2.COLOR_BGR2RGB)])
        #print(len(tiles))
    ...
    def tile_augumentation(self, tiles):
        ...
        if np.random.rand() > 0.5:
            for i, tile in enumerate(tiles):
                rotate_indx = np.random.randint(0,4)
                if rotate_indx == 3:
                    continue
        ...
        if np.random.rand() > 0.5:
            for i, tile in enumerate(tiles):
                ...
                tiles[i] = cv2.flip(tile,flip_indx)
    ...
    def get_image(self, index):
        ...
        if self.phase == 'train':
            #pass
            #augumentation
            tiles = self.tile_augumentation(tiles);            
